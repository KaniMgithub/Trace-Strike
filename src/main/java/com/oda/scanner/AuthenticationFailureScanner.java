...
public class AuthenticationFailureScanner implements VulnerabilityScanner {

    @Override
    public String getScannerName() {
        return "Authentication Failures Scanner (A07)";
    }

    @Override
    public List<Finding> scan(CompilationUnit cu, String fileName) {
        List<Finding> findings = new ArrayList<>();
        cu.accept(new AuthVisitor(findings, fileName), null);
        return findings;
    }

    private static class AuthVisitor extends VoidVisitorAdapter<Void> {
        private final List<Finding> findings;
        private final String fileName;
        private final TaintTracker tracker = new TaintTracker();

        AuthVisitor(List<Finding> findings, String fileName) {
        ...
        public void visit(MethodCallExpr n, Void arg) {
            ...
            if ("verifyJuiceCredentials".equals(name) || "checkPassword".equals(name) || "authenticateUser".equals(name)) {
        ...
        private void checkSink(String argsStr, int sinkLine, String sinkMethod, String sinkSnippet) {
            for (String tainted : tracker.getTaintedVars()) {
                ...
                findings.add(new Finding(
                        Finding.VulnType.AUTHENTICATION_FAILURE,
                        fileName,
                        sinkLine,
                        tainted,
                        httpParam,
                        ...
                        RemediationLibrary.forType(Finding.VulnType.AUTHENTICATION_FAILURE)
                ));
                return;
            }
        }
